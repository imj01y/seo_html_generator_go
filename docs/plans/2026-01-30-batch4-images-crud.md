...
**Goal:** 实现图片管理的 17 个 API 接口，包括分组管理、URL管理、批量操作
...
- Batch 1-3 已完成（auth, dashboard, logs, templates, keywords 模块）
- 数据库表 `image_groups` 和 `images` 已存在
...
| PUT | /api/images/groups/:id | 更新分组 |
| DELETE | /api/images/groups/:id | 删除分组 |

### 图片URL管理 (5个)
| 方法 | 路径 | 功能 |
|------|------|------|
| GET | /api/images/urls/list | 获取图片URL列表（分页） |
| POST | /api/images/urls/add | 添加单个图片URL |
| POST | /api/images/urls/batch | 批量添加图片URL |
| PUT | /api/images/urls/:id | 更新图片URL |
| DELETE | /api/images/urls/:id | 删除图片URL |

### 批量操作 (4个)
| 方法 | 路径 | 功能 |
|------|------|------|
| DELETE | /api/images/batch | 批量删除 |
| DELETE | /api/images/delete-all | 删除全部 |
| PUT | /api/images/batch/status | 批量更新状态 |
| PUT | /api/images/batch/move | 批量移动分组 |

### 统计和缓存 (4个)
| 方法 | 路径 | 功能 |
|------|------|------|
| GET | /api/images/urls/stats | 获取统计信息 |
| GET | /api/images/urls/random | 随机获取图片URL |
| POST | /api/images/urls/reload | 重新加载 |
| POST | /api/images/cache/clear | 清理缓存 |
...
**Files:**
- Create: `go-page-server/api/images.go`
...
```bash
git add go-page-server/api/images.go
git commit -m "feat(api): 添加图片模块结构体定义"
```
...
**Files:**
- Modify: `go-page-server/api/images.go`

**Step 1: 删除临时变量，实现分组 CRUD**

删除文件末尾的临时变量部分，添加以下方法：
...
```bash
git add go-page-server/api/images.go
git commit -m "feat(api): 实现图片分组管理接口"
```
...
**Step 1: 实现列表、添加、更新、删除**

在 DeleteGroup 方法之后添加：
...
```bash
git add go-page-server/api/images.go
git commit -m "feat(api): 实现图片URL管理接口"
```
...
```bash
git add go-page-server/api/images.go
git commit -m "feat(api): 实现图片批量操作接口"
```
...
```bash
git add go-page-server/api/images.go
git commit -m "feat(api): 实现图片统计和缓存接口"
```
...
git commit -m "feat(api): 注册图片管理路由 (17个接口)"
```
...
**Files:**
- Create: `go-page-server/api/images_test.go`
...
Run: `cd go-page-server && go test ./api -run TestImages -v`

**Step 3: Commit**

```bash
git add go-page-server/api/images_test.go
git commit -m "test(api): 添加图片 API 测试"
```
...
Run: `cd go-page-server && go build -o go-page-server.exe .`

**Step 3: 最终 Commit**

```bash
git add -A
git commit -m "feat: Batch 4 完成 - 图片管理模块 (17 接口)"
```
