...
class ImageCachePool:
    ...
    async def resize(self, new_size: int) -> None:
        ...
        logger.info(f"ImageCachePool resized: {old_size} -> {new_size}, low_watermark: {self._low_watermark}")
        ...
        if new_size > old_size:
            ...
            with self._consume_lock:
                ...
                if current_remaining > new_size:
                    ...
                    self._url_set = set(remaining)
                    self._cursor = 0
                    logger.info(f"ImageCachePool truncated to {len(self._cache)} URLs")
